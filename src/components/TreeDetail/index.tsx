...
const buildAddressFromComponents = (components: AddressComponent[]) => {
  ...
  const sublocality =
    components.find((component) =>
      component.types.find((type) => type === "sublocality"),
    )?.["long_name"] || "";
  const adminArea =
    components.find((component) =>
  ...
  const plusCode =
    components.find((component) =>
      component.types.find((type) => type === "plus_code"),
    )?.["long_name"] || "";
  const locality =
    components.find((component) =>
  ...
  if (streetNumber && route) {
    ...
    return [plusCode, locality + ", " + adminArea];
  } else {
    return null;
  }
};
...
export default function TreeDetail() {
  ...
  const selectedTreeLatLng = useMemo(
    () =>
      selectedTree
        ? {
  ...
  const { data: locationData, isLoading: locationLoading } = useGeocode(
    selectedTreeLatLng || "",
  );

  const address = useMemo((): string[] | null => {
    return locationData
      ? buildAddressFromComponents(locationData.data[0]["address_components"])
      : null;
  }, [locationData]);
    ...
    <Box sx={{ height: "100%", paddingLeft: 0, paddingRight: 0 }}>
      <Box sx={{ pb: 3, pt: isMobile ? 1 : 3 }}>
        ...
        {selectedTree?.species && (
          ...
          >
            {typedSpeciesDetails[selectedTree?.species].commonNames}
          </Typography>
        )}
        <Typography
          component="div"
        ...
        >
          <p className="w-full flex justify-between">
            ...
            >
              ...
              {selectedTree?.structure === "Stump" && " (stump)"}
            </span>
          </p>
